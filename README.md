...
- 오프체인 분석 결과 저장 및 FastAPI AI 연동 adapter
...
- `DOCUMENT_AI_ENABLED=true`이면 오프체인 분석 API가 sanitized payload를 FastAPI `/document-analysis`로 전송하고, 비활성 상태에서는 placeholder 결과를 저장합니다.
- 오프체인 분석에서 원문 파일과 필터링 전 OCR 결과는 AI 레이어로 전달하지 않으며, 세부 입력 규격은 `docs/offchain-analysis-contract.md`를 따릅니다.
- 챗봇은 문서 분석과 별도 플로우이며, Spring `POST /api/ai/chat/stream`이 FastAPI `/chat/stream` SSE를 프록시합니다.
...
- `분석 요청`으로 OCR 추출 payload hash 기반 분석 결과가 저장되는지
...
9. `분석 요청`으로 OCR 추출 payload hash 기반 분석 저장 확인
